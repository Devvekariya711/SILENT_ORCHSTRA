...
const Lobby: React.FC<LobbyProps> = ({ onSelect, onEnterRoom, onLeaveRoom, roomId: propRoomId, isInRoom }) => {
    const [localRoomId, setLocalRoomId] = useState(propRoomId || '');
    const [selectedRole, setSelectedRole] = useState<InstrumentRole | null>(null);
    // If already in room, start at INSTRUMENT step
    const [step, setStep] = useState<'ROOM' | 'INSTRUMENT'>(isInRoom ? 'INSTRUMENT' : 'ROOM');
    const [error, setError] = useState<string | null>(null);

    // Use prop roomId if provided
    const roomId = isInRoom && propRoomId ? propRoomId : localRoomId;
    ...
    const instruments = [
        ...
        { id: InstrumentRole.THEREMIN, label: 'THEREMIN', color: 'border-pink-500 text-pink-400', desc: 'Wave hands through the air for eerie continuous tones.' },
        { id: InstrumentRole.STRINGS, label: 'STRINGS', color: 'border-emerald-500 text-emerald-400', desc: 'Conduct sweeping orchestral string sections.' },
        { id: InstrumentRole.PADS, label: 'AMBIENT PADS', color: 'border-indigo-500 text-indigo-400', desc: 'Create atmospheric soundscapes with hand movements.' },
    ];
        ...
        <div className="flex flex-col items-center h-full p-6 overflow-y-auto pt-10 pb-32 w-full">
                        ...
                        <div className="flex flex-col md:flex-row gap-8 items-center justify-between">
                            ...
                            <div className="flex-1 w-full max-w-sm">
                                ...
                                <input
                                    ...
                                    value={localRoomId}
                                    onChange={(e) => {
                                        setLocalRoomId(e.target.value.toUpperCase());
                                        setError(null);
                                    }}
                                    maxLength={6}
            ...
            {step === 'INSTRUMENT' && (
                <>
                    ...
                    <button
                        onClick={() => {
                            if (isInRoom && onLeaveRoom) {
                                // If we came from Stage, leave room completely
                                onLeaveRoom();
                            } else {
                                // Otherwise just go back to room selection
                                setStep('ROOM');
                            }
                        }}
                    ...
                    >
                        {isInRoom ? 'Leave Room' : 'Change Room'}
                    </button>
